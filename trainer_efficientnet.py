...
from models import resnet,vgg,wrn,resnet50, efficientnet
...
model_names = ["vgg", "wrn", "resnet", "efficientnet"]
...
def main():
    ...
    model = efficientnet.EfficientNet.from_name("efficientnet-b0", num_classes=10)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=5e-2, momentum=0.9)

    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs, verbose=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            }, is_best, filename=os.path.join(args.save_dir, 'efficientnet_proposed.th'))

    if args.evaluate:
        filename=os.path.join(args.save_dir, 'efficientnet_baseline.th')
        checkpoint = torch.load(filename)
        model.load_state_dict(checkpoint['state_dict'])
        test(val_loader, model, criterion,normalised=None)
        return
